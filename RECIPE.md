...
| Provider scope | Anthropic + xAI native, **+ Google Gemini** | 0.90 | Originally "Anthropic + xAI native only" (drop the generic OpenAI/multi-provider crate). **Relaxed 2026-06-12** at the owner's request: added `lvz-google` (Gemini, native Generative Language API over SSE) to enable same-model benchmarking vs. agents on `gemini-3-flash-preview` (`bench/README.md`). Still a hand-rolled thin adapter, no SDK. OpenAI and others remain out of scope. |
