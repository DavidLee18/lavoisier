...
| `lvz-gw-acp` | **ACP (Agent Communication Protocol, BeeAI/IBM) server** gateway (axum): the REST agents/runs API — `GET /agents`, `POST /runs` (sync / SSE-stream / async), `GET /runs/{id}`, cancel. ACP `session_id` ↔ session. Hand-rolled REST; optional API-key auth. |
