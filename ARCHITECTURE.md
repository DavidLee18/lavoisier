...
| `lvz-legion` | The **legion council** (library): a `Panel` of provider+model `Debater`s that argue a task out — draft → critique → judge — and return one agreed plan. Implements `lvz-protocol`'s `Deliberator`; the agent runs it as a pre-pass (deliberate-then-act) that supersedes the single advisor. Leaf crate: depends only on `lvz-protocol`. |
