...
6. Release notes are generated by the workflow, not written at tag time: `generate_release_notes:
   true` plus a fixed body that links **this tag's** `MIGRATING.md`. Immutable releases mean the
   notes cannot be corrected after the fact either, so there is nothing to remember to paste in.

7. The workflow builds three targets — `aarch64-apple-darwin`, `x86_64-unknown-linux-gnu`,
...
8. **Verify the published artifact, not just the green run.** Download the tarball and run it:
