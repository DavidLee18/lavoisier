...
//!
//! `RECIPE.md` (§1, §8) gives xAI two transports: a gRPC primary path (codegen from
//! `xai-proto`, lands at M7) and an **in-crate** OpenAI-compatible fallback against
//! `https://api.x.ai/v1`. This crate currently implements the fallback behind the
//! [`Provider`] contract. It streams (`stream: true`), normalising SSE chunks into the
//! [`Event`] stream, and supports OpenAI-style function/tool calling: tool definitions are
//! sent, normalised tool blocks are mapped into OpenAI message shape, and streamed
//! `tool_calls` deltas are decoded into [`Event::ToolUseStart`]/`ToolUseDelta`/`ToolUseEnd`.
//!
//! The fallback has no prompt caching and no native server-side tools, so those
//! [`Capabilities`] are `false`; parallel tool use is supported.
...
impl XaiProvider {
    ...
    /// Construct from the environment: `XAI_API_KEY` (required) and `XAI_BASE_URL` (optional).
